...
the potential use of a weight vs age curve. Weight (W) as a function of age (A) 
...
function can be written as: $$ Age_{t} = (W_{t} /\alpha)^{1/\beta} $$

$$ Age = W_{t+1} = α(Age_{t} + 7 days)^{β} $$. Together these
functions can be used to compute the projected growth of a cohort sampled by the 
fishery in any given week. The projected growth of an squid of size W(t) to
size W(t+1) in week t+1 was obtained by first estimating its age in days, then
adding 7 days to project it to the next week.  
...
where W` is the predicted length-specific mean weight for the fish population in a given region
...
## ------ NOTE -------- ##
# Look at relative condition by week or month 
