...
lw_data <- read.csv('ILXSM_EntirePull_Wdealerinfo_3_14_21.csv')
...
The following plots show the breakdown of the number of vessels contributing samples each year and as well as the number of vessels across each of the participating processors 
...
dat_tally2 <- dat %>% 
  group_by(year, DEALER_NAME) %>%
  summarise(n_vessels = length(unique(VESSEL_NAME)), 
            n_stat_areas = length(unique(AREA_CODE)))



p1 = ggplot(data = dat_tally, aes(x = factor(year), y = n_vessels, 
                                  fill = factor(year))) +
...
p2 = ggplot(data = dat_tally2, aes(x = factor(year), y = n_vessels, 
                                   fill = factor(DEALER_NAME))) +
  geom_bar(stat ='identity', position = position_dodge(),
           col = 'black') +
  ylab('Number of Vessels') +
  xlab('Year') +
...
At the time of this report, the number of individual squid with 
...
> Here are some maps highlighting the statistical areas from which squid
were sampled.


+ The first figure depicts the boundaries of the Greater Atlantic Region 
Statistical Areas from which Illex samples have been reported to the ILXSM database.

+ The second figure indicates the amount of samples reported for each statistical 
area. The size of the data point is representative of the number of samples recorded that particular area. Note this map is cumulative, it summarizes total 
samples in each stat area across all available years. 
...
Density estimates for weight and length data. The three lines corresponding to
the first (lower quartile), second (median), and third quartile (upper quartile):
...
ggplot(wt_full %>% filter(year == 2021), aes(x = weight, y = factor(week),
  ...
  labs(title = '2021 Illex Weights',
       x = 'Weight (gm)', y = 'Week') +
  scale_fill_brewer(name = 'Quartiles', palette = 'YlOrBr') +
  ecodata::theme_ts()
...
ggplot(wt_full %>% filter(year == 2022), aes(x = weight, y = factor(week),
  ...
  labs(title = '2022 Illex Weights',
       x = 'Weight (gm)', y = 'Week') +
  scale_fill_brewer(name = 'Quartiles', palette = 'Purples') +
  ecodata::theme_ts()
...
ggplot(wt_full %>% filter(year == 2023), aes(x = weight, y = factor(week),
  ...
  labs(title = '2023 Illex Weights',
       x = 'Weight (gm)', y = 'Week') +
  scale_fill_brewer(name = 'Quartiles', palette = 'PuRd') +
  ecodata::theme_ts()
...
+ The data set ranges from 1997-2006 to 2009-2023. There is no data for 
years 2007 and 2008.
...
+ Weight range: **`r min(bwts$BODYWT)`** grams to **`r max(bwts$BODYWT)`** grams
...
*Note: To better fit all years on the x-axis only the last two digits of each* 
*year are labeled. For example, 1997 is labeled '97' and 2019 is labeled '19'*
```{r}
...
# classic plot :
g3 <-ggplot(bwts, aes(x = factor(year), y = BODYWT)) + 
  labs(x = 'Year',
       y = 'Weight (g)') +
  geom_boxplot() +
  geom_point(data = bwts[bwts$BODYWT,], alpha = 0) +
  ecodata::theme_ts() +
  guides(fill=guide_legend(title=NULL)) +
  scale_x_discrete(
    breaks = c(1997:2006, 2009:2023), 
...
Extending work by Paul Rago in 2021, size samples from Lisa's data and 
ILXSM were used to illustrate the potential use of a weight vs age curve. 
...
Condition estimates are often based on length-weight relationships. Below is a
...
in effort to compare to length-weight relationships based on 1978 Lange and 
Johnson study.
...
g2 = ggplot(lw_paired %>% filter(year %in% c(2021,2022)), 
...
# ggplot(lw_paired %>% filter(year %in% c(2021,2022)), aes(x=length, y=weight)) +
#   geom_hex() +
#   scale_fill_gradientn(colours=r) + 
#   xlab('Mantle Length (mm)') + 
#   ylab('Body Weight (g)') + 
...
Regression coefficients used to compute Kn in plots below are 
based on those identified in 1978 Lange and Johnson study.
...
'Freeze' represents freezer trawlers and 'Wet' represents vessels with 
...
+ The first plot is during the fishing season window, here estimated as weeks
20 through 40: 
