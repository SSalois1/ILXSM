...
library(sf)
library(sp)
ml <- read.csv('ilxsm_ml.csv')
wt <- read.csv('ilxsm_wt.csv')
dat <- read.csv('ILXSM_condition_20212022.csv')
ml_tally <- read.csv('ml_tally.csv')
...
stat_areas2 <- stat_areas[na.omit(stat_areas@data$Id %in% c('622', '537', '632',
                                                            ...
                                                            '626', '613', '621', 
                                                            '600', '562')),]
#stat_areas2 <- stat_areas2 %>% st_as_sf()
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(input = "Year", label= "Year:",
                  choices = unique(ml_tally$year), selected ="2021"),
      selectInput('var2', 
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Highlighting Maps",
                           fluidRow(
                             bscols(plotlyOutput("highlighthist"), plotlyOutput("map2"))
                           ),
                           fluidRow(
                             bscols(plotlyOutput("highlighthist2"), plotlyOutput("map3"))
                           ),
                  ),
...
server <- function(input, output) {
  ...
  output$map <- renderPlotly({
    ...
    # )
    # fig = plot_geo(data=filter(ml_tally, year == input$Year)) %>%
    #   add_trace(
    #     x = ~lon, y = ~lat, size = ~n, 
    #     type = 'scatter',
    #     mode = 'markers', 
    #     marker = list(size = 10),
    #     text = ~paste('Statistical Area: ', AREA_CODE, '<br />',
    #                   'Lengths','<br />',
    #                   'Mean:', mlength, 'mm', '<br />',
    #                   'Sd:', sdlength, 'mm', '<br />',
    #                   'Max:', maxlength, 'mm', '<br />',
  ...
  output$highlighthist <- renderPlotly({
    ...
    # histogram length
    length_hist <- plot_ly(ml1, x = ~ PARAM_VALUE_NUM) %>%
  ...
  output$highlighthist2 <- renderPlotly({
    ...
    # histogram length
    length_hist <- plot_ly(ml2, x = ~ PARAM_VALUE_NUM) %>%
      add_histogram(xbins = list(start = 0, end = 500)) %>%
      layout(barmode = "overlay",
             title = '2022',
             xaxis = list(title = 'Mantle length (mm)'), 
  ...
  output$boxplot_ml1 <- renderPlot({
    ## Create boxplot by fleet, color = factor(year)
    # removed outliers - need to fix cm to mm conversions
    ggplot(ml %>% filter(hold_type == 'FT'& PARAM_VALUE_NUM <= 500), 
           aes(x = factor(vessel_id), y = PARAM_VALUE_NUM, fill = as.factor(year)))+ 
  ...
  output$boxplot_ml2 <- renderPlot({
    ggplot(ml %>% filter(hold_type %in% c('RSW', 'Ice')& PARAM_VALUE_NUM <= 500), 
           aes(x = factor(vessel_id), y = PARAM_VALUE_NUM, fill = as.factor(year)))+ 
      labs(x = 'Vessel', y = 'Mantle length (mm)', 
           title = 'Fleet: Wet boats', 
           subtitle = 'Lengths') +
